...
// ── Lesson Slide onCall wrappers (Gen 1) ─────────────────────────────────────
// These match the existing deployed function names so they UPDATE existing functions.
// Client uses fetch('/api/generate-slide') -> generateSlide (onRequest above).
// These onCall versions are kept as fallback for the existing mobile/browser callable SDK.
exports.generateLessonSlide = httpsV1.onCall(async (data, context) => {
  const { subject, grade, topic, preserveLanguage } = data || {};
  if (!subject || !grade || !topic) {
    throw new functionsV1.https.HttpsError('invalid-argument', 'subject, grade, and topic are required');
  }
  if (!process.env.ANTHROPIC_API_KEY) {
    throw new functionsV1.https.HttpsError('failed-precondition', 'ANTHROPIC_API_KEY not configured');
  }
  let userMsg = "Subject: " + String(subject).slice(0, 60) + "\nGrade: " + String(grade).slice(0, 10) + "\nTopic: " + String(topic).slice(0, 200);
  if (preserveLanguage && String(preserveLanguage).trim()) {
    userMsg += "\nPreserve this specific language if relevant: \"" + String(preserveLanguage).slice(0, 100) + "\"";
  ...
  const run = async () => {
    const apiData = await anthropicPost(process.env.ANTHROPIC_API_KEY, {
      model: 'claude-haiku-4-5-20251001',
      max_tokens: 512,
      system: SLIDE_SYSTEM_PROMPT,
      messages: [{ role: 'user', content: userMsg }],
  ...
  try { return await run(); } catch (e1) {
    ...
    try { return await run(); } catch (e2) {
      console.warn('generateLessonSlide attempt 2:', e2?.message);
      throw new functionsV1.https.HttpsError('unavailable', 'fill in manually');
    }
  }
});

exports.simplifyLessonSlide = httpsV1.onCall(async (data, context) => {
  const { grade, learningTarget, outcomes } = data || {};
  if (!grade || !learningTarget) {
    throw new functionsV1.https.HttpsError('invalid-argument', 'grade and learningTarget are required');
  }
  if (!process.env.ANTHROPIC_API_KEY) {
    throw new functionsV1.https.HttpsError('failed-precondition', 'ANTHROPIC_API_KEY not configured');
  }
  ...
  try { return await run(); } catch {
    throw new functionsV1.https.HttpsError('unavailable', 'simplify unavailable');
  }
});
