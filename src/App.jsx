...
/* ── data ── */
const CAT_META = {
  "Greeting":       { color: "#7EC8A4", dark: "#2A7A54", emoji: "👋" },
  "Sharing":        { color: "#7AACDA", dark: "#2A5A8A", emoji: "💬" },
  "Group Activity": { color: "#F2C06E", dark: "#8A620A", emoji: "✦" },
  "Morning Message":{ color: "#B09FDB", dark: "#4A3A8A", emoji: "✎" },
  "SEL Prompt":     { color: "#7AACDA", dark: "#2A5A8A", emoji: "💬" },
  "Brain Teaser":   { color: "#F2C06E", dark: "#8A620A", emoji: "🧩" },
  "Movement Break": { color: "#F2906E", dark: "#8A3A1A", emoji: "🏃" },
  "Mindfulness":    { color: "#B09FDB", dark: "#4A3A8A", emoji: "🌿" },
  "Vocabulary":     { color: "#5BA7E1", dark: "#1F5E8A", emoji: "Aa" },
  "Math Do Now":    { color: "#66B76A", dark: "#256C2A", emoji: "∑" },
  "Writing Do Now": { color: "#D08B62", dark: "#8A3F1B", emoji: "✍" },
  "On This Day":    { color: "#D6A94D", dark: "#8A620A", emoji: "⏳" },
...
function getProjectorBackgroundImage(style) {
  const preset = PROJECTOR_BACKGROUNDS[style.backgroundPreset] ? style.backgroundPreset : "Solid";
  if (preset === "CustomUrl") {
    return style.backgroundUrl ? `linear-gradient(rgba(0,0,0,${style.overlayOpacity / 100}), rgba(0,0,0,${style.overlayOpacity / 100})), url("${style.backgroundUrl}")` : "none";
  }
  const image = PROJECTOR_BACKGROUNDS[preset]?.image || "none";
  return image === "none" ? "none" : `linear-gradient(rgba(0,0,0,${style.overlayOpacity / 100}), rgba(0,0,0,${style.overlayOpacity / 100})), ${image}`;
}

function normalizeProjectorStyle(style = {}, account = null) {
...
function readProjectorStyle(account = null) {
  try {
...
function persistProjectorStyle(style) {
...
function readPresentationView() {
  try {
    const value = localStorage.getItem(PRESENTATION_VIEW_KEY);
    return value === "guided" ? "guided" : "clean";
  } catch {
    return "clean";
  }
}

function readProjectorState() {
  try {
    return JSON.parse(localStorage.getItem(PROJECTOR_STATE_KEY) || "null");
  } catch {
    return null;
  }
}
...
function ProjectorReceiver() {
  const [state, setState] = useState(() => readProjectorState());

  useEffect(() => {
    const update = () => setState(readProjectorState());
  ...
  const closeProjector = useCallback(() => {
    ...
    window.close();
    setState(readProjectorState());
  }, []);

  if (!state?.active || !Array.isArray(state.routine) || !state.routine.length) {
    return (
      <div className="display-mode projector-waiting">
        <div className="disp-center">
          <div className="disp-prompt">Projector is ready.</div>
          <div className="disp-starter-box">
            <div className="disp-starter-label">Waiting</div>
            <div className="disp-starter-text">Choose something to project from the teacher screen.</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <DisplayMode
      ...
      projectorStyle={state.projectorStyle || DEFAULT_PROJECTOR_STYLE}
      initialView={state.presentationView || "clean"}
      onExit={closeProjector}
    />
  );
}
